    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">Mẫu nội dung</h1>
                    <p className="text-sm text-gray-500">Quản lý các mẫu Email, SMS, Telegram và Zalo</p>
                </div>
                <button
                    onClick={openCreateModal}
                    className="px-6 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-500/30 transition-all font-medium flex items-center gap-2"
                >
            ...
            <ContentTemplateFilter initialFilters={filters} onUpdateFilters={updateFilters} />
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50/50">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Mẫu / Mã</th>
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Phân loại</th>
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Loại / Kênh</th>
                                ...
                                {items.length > 0 ? (
                                    items.map((item: ContentTemplate, index) => (
                                        <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-2.5 py-1 text-[10px] font-bold uppercase rounded-md ${item.category === 'render' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
                                                    }`}>
