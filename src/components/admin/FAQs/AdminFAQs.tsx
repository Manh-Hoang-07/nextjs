...
export default function AdminFAQs({
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(adminEndpoints.faqs.list, { params });
  ...
  const openEditModal = async (item: FAQ) => {
    try {
      const response = await api.get(adminEndpoints.faqs.show(item.id));
  ...
  const handleCreated = async (formData: any) => {
    try {
      await api.post(adminEndpoints.faqs.create, formData);
  ...
  const handleUpdated = async (formData: any) => {
    ...
    try {
      await api.put(adminEndpoints.faqs.update(selectedItem.id), formData);
  ...
  const deleteItem = async () => {
    ...
    try {
      await api.delete(adminEndpoints.faqs.delete(selectedItem.id));
