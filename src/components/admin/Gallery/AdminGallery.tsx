...
export default function AdminGallery({
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(adminEndpoints.gallery.list, { params });
  ...
  const openEditModal = async (item: Gallery) => {
    try {
      const response = await api.get(adminEndpoints.gallery.show(item.id));
      const data = response.data?.data || response.data;
  ...
  const confirmDelete = (item: Gallery) => {
    setSelectedItem(item);
  ...
  const handleCreated = async (formData: any) => {
    try {
      await api.post(adminEndpoints.gallery.create, formData);
  ...
  const handleUpdated = async (formData: any) => {
    ...
    try {
      await api.put(adminEndpoints.gallery.update(selectedItem.id), formData);
  ...
  const deleteItem = async () => {
    ...
    try {
      await api.delete(adminEndpoints.gallery.delete(selectedItem.id));
    ...
    <div className="admin-gallery">
      ...
      {modals.delete && selectedItem && (
        <ConfirmModal
          show={modals.delete}
          title="Xác nhận xóa"
          message={`Bạn có chắc chắn muốn xóa ${selectedItem.title || selectedItem.name || ""}?`}
